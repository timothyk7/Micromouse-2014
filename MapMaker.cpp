...
void MapMaker::helpTopBot(int row, int wall [MAX_SIZE][MAX_SIZE])
{
    (void)printf("\t");
    int col;
    for(col = 0; col<MAX_SIZE; col++)
    {
       if(((wall[row][col]>>2)&0x0001) == 0x0001)//if theres a top wall
                (void)printf(" ____ ");
       else
                (void)printf("      ");//3 orig
    }
...
/*Public*/
Info MapMaker::getMapInfo(int x, int y)
{
     return mazeMap[x][y];
}
 ...
 /*
     ...
     * param 3: wall setup
     */
 void MapMaker::initialize(const char *maze)
 {
 ...
 * Display the map of the full maze
   param: 
          ...
          5 direction
 */
void MapMaker::displayMaze(int num, int number[MAX_SIZE][MAX_SIZE], int cX, int cY, char dir)
      {
        (void)printf("\t ");
        int x = 0;
 ...
 * Display the map of the current bot maze
   param: 
          ...
          6 direction
 */
void MapMaker::displayBotMaze(int num, int number[MAX_SIZE][MAX_SIZE], int map[MAX_SIZE][MAX_SIZE], int cX, int cY, char dir)
      {
        (void)printf("\t ");
        int x = 0;
        for (x=0; x<MAX_SIZE; x++)
        ...
        for (row = 0; row < MAX_SIZE; row++)
        {
          ...
          for (col =0; col < MAX_SIZE; col++)
          {
              if(((map[row][col]>>1) & 0x0001) == 0x0001)//if theres a left wall
                (void)printf("|");
              else
                (void)printf(" ");
              if(cX == row && cY == col){
              ...
              if((map[row][col]&0x0001) == 0x0001)//if theres a rt wall
                (void)printf("|");
              else
                (void)printf(" ");
          }
